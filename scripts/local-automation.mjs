...
export function startAutomation(enabled){
 ...
 const status={enabled:active,mode:'Local preview',analysisIntervalHours:6,dailyTime:'07:00 IST',nextAnalysis:active?new Date(Date.now()+21600000).toISOString():null,nextDaily:active?new Date(nextDaily()).toISOString():null,running:false,lastResult:null};
 let child;
 ...
 const timer=active?setInterval(async()=>{if(status.running)return;const daily=Date.parse(status.nextDaily)<=Date.now(),analysis=Date.parse(status.nextAnalysis)<=Date.now();if(!daily&&!analysis)return;status.running=true;try{await run('./collect-public.mjs');await run('./generate-impact.mjs');await run('./generate-intelligence.mjs',['--type='+(daily?'daily':'analysis')]);status.lastResult={ok:true,at:new Date().toISOString()};}catch{status.lastResult={ok:false,at:new Date().toISOString(),message:'Refresh failed; previous reports retained'};}finally{status.running=false;if(daily)status.nextDaily=new Date(nextDaily()).toISOString();else status.nextAnalysis=new Date(Date.now()+21600000).toISOString();}},30000):null;
