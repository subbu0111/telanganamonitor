...
export function indiaRelevant(a){if(a.sourceScope==='world')return false;if((a.locations||[]).some(l=>l.country==='India'&&l.state))return true;if(/\b(india|indian|telugu|tamil|bollywood|tollywood|kollywood)\b/i.test(a.title+' '+(a.excerpt||'')))return true;try{return /\/(india|national|nation|cities|telangana|andhra-pradesh|tamil-nadu|hyderabad)(\/|[?#])/.test(new URL(a.url).pathname);}catch{return false;}}
