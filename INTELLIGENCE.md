...
Public reports use only records from the approved reuse registry. See PUBLIC-RELEASE.md for source conditions and the GitHub Actions schedule. Generated reports are stored in `data/intelligence.json`. The private local archive is excluded from publishing. Static GitHub Pages cannot run this local scheduler or hold the model credential; GitHub Actions performs unattended generation using the existing repository secret.
